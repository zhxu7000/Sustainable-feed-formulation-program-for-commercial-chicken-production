...
const defaultData = [
    { name: 'Material A', value: 120 },
    { name: 'Material B', value: 300 },
    { name: 'Material C', value: 500 },
];

// CostPieChart组件
const CostPieChart = ({ recipeId }) => {
    const [chartData, setChartData] = useState(defaultData); // 使用 defaultData 作为初始值
            ...
            try {
                ...
                if (Array.isArray(response)) {
                    const transformedData = response.map(item => ({
                        ...
                        value: parseFloat(item.total_cost.slice(1)) // 移除$符号并转换为浮点数
                    }));
